...
export function NewMessage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!draft.trim() || selectedRels.length === 0) return;
    ...
    try {
      ...
      await Promise.all(
        selectedRels.map(async (rel) => {
          try {
            const res = await rewriteMessage('new', draft, rel);
            newResults[rel] = res;
          } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder="伝えたい内容をラフに入力してください..."
            className="w-full h-32 glass-input rounded-2xl p-4 text-white placeholder:text-white/30 resize-none text-base"
            required
          />
        </div>
        ...
        <button
          type="submit"
          disabled={loading || !draft.trim() || selectedRels.length === 0}
